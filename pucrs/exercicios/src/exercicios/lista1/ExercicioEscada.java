...
public class ExercicioEscada extends RunnableExercise {
	...
	public void run(Scanner in) {
		...
		do {
			try {
				System.out.println(
					"\nVamos calcular, com base na altura e quantidade de degraus desta escada, quantos degraus uma pessoa " +
					"precisa subir " + "para atingir determinada altura e qual a altura máxima que pode ser atingida");
				...
				if (alturaAtual < ALTURA_MINIMA_PESSOA || alturaAtual > ALTURA_MAXIMA_PESSOA) {
					throw new IllegalArgumentException("A altura atual deve estar entre %.2f m e %.2f m!".formatted(
						ALTURA_MINIMA_PESSOA,
						ALTURA_MAXIMA_PESSOA
					));
				}
		...
		msg += degrausParaAlturaDesejada > quantidadeDegraus
			? "seria preciso subir %.0f degraus, %d a mais do que a escada possui".formatted(degrausParaAlturaDesejada,
			Math.round(degrausParaAlturaDesejada - quantidadeDegraus)
